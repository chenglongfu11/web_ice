...
ida_lib.compareResults.restype = ctypes.c_long
ida_lib.compareResults.argtypes = [ctypes.c_char_p, ctypes.c_char_p, ctypes.c_long, ctypes.c_char_p, ctypes.c_int]
ida_lib.export3DView.restype = ctypes.c_long
ida_lib.export3DView.argtypes = [ctypes.c_long, ctypes.c_char_p, ctypes.c_long, ctypes.c_char_p, ctypes.c_int]
ida_lib.exportDiagram.restype = ctypes.c_long
ida_lib.exportDiagram.argtypes = [ctypes.c_long, ctypes.c_char_p, ctypes.c_long, ctypes.c_char_p, ctypes.c_int]
ida_lib.importIFCFile.restype = ctypes.c_long
ida_lib.importIFCFile.argtypes = [ctypes.c_char_p, ctypes.c_long, ctypes.c_long, ctypes.c_long, ctypes.c_char_p, ctypes.c_int]
...
def call_ida_api_function2(fun, *args):
  "Just send in the function name and its unique arguments (not out buffer and out buffer length)"
  p = ctypes.create_string_buffer(5000)
  args = args + (p,len(p))
  res = fun(*args)
  if res == 0:
    return ida_poll_results_queue(2)
  elif res > 0 :
    p = ctypes.create_string_buffer(res)
    res = fun(*args)
    if res == 0:
